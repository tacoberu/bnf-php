...
class SequenceTest extends TestCase
{
	...
	function testSample8()
	{
		...
		$parser = new Sequence('Collection', [
			new Match_('CollectionStart', ['['], False),
			new Variants(Null, [
				$element,
				$sep
			]),
			new Match_('CollectionEnd', [']'], False),
		]);
	...
	function testUnmatch4()
	{
		...
		$parser = new Sequence(Null, [
			...
			$sep,
			new Match_('start-bracket', ['[']),
			new Variants(Null, [
				new Numeric('vals'),
				new Pattern(Null, ['~\s*,\s*~']),
			]),
			new Match_('end-bracket', [']']),
		]);
		list($token, $expec) = $parser->scan($src, 0, []);
		$this->assertFalse($token);
		$this->assertSame([
	...
	function testFail5()
	{
		$this->expectException(LogicException::class);
		$this->expectExceptionMessage("Sequence combinator must containt minimal two items.");
		new Sequence(Null, [new Match_(Null, [])]);
	}
